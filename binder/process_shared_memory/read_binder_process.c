...
int main() {
    ...
    ashId = ashmem_create_region(OEM_SHARED_MEMORY_NAME,
                  OEM_SHARED_MEMORY_SIZE);
    if (ashId < 0) {
    ...
    buffer = (char*)mmap(NULL, OEM_SHARED_MEMORY_SIZE, 
                  PROT_READ | PROT_WRITE, MAP_SHARED, ashId, 0); 
    if (buffer < 0) {
